...
app.controller("EntitlementController", ["$scope", "entitlementService", "$modal", "$rootScope", "$timeout", function($scope, entitlementService, $modal, $rootScope, $timeout) {
	...
	$scope.getFolios = function() {
		entitlementService.getFolios($scope.guid).then(function(folios) {
			// if list if empty, initialize an empty array
			$scope.folios = folios || [];
	...
	if (href.indexOf("index.php") || href.indexOf("index.html")) {
		href = href.replace(/index.php|index.html/i, '');
	} else {
		href += (href.lastIndexOf("/") == href.length - 1 ? "" : "/");
	}

	$scope.serviceURL = href + "services/index.php";
	...
	$scope.folioGridOptions = {
		...
		columnDefs: [
			{displayName: "Availability Date", field: "publicationDate", cellFilter: "date", width: 120},
			{displayName: "Product Label", field: "magazineTitle"},
			{displayName: "Product Description", field: "issueNumber"},
	...
	$scope.displayAddFolioDialog = function() {
		var modalInstance = $modal.open({
			templateUrl: "js/partials/AddFolioDialog.html",
			controller: AddFolioDialogController,
			size: "sm",
			resolve: {
				entitlementService: function() { return entitlementService; },
				folio: function() { return $scope.selectedFolio; },
		...
		modalInstance.result.then(
			function(data) { // Handler for when a user clicks "ok".
				$scope.folios.push({publicationDate:data.pubDate, magazineTitle:data.pubName, issueNumber:data.folioNumber, productId: data.productId});
			}
		);
	};
	...
	$scope.deleteFolio = function() {
		if ($scope.selectedFolio && !$scope.isDeletingFolio) {
			$scope.isDeletingFolio = true;
			entitlementService.deleteFolio($scope.guid, $scope.selectedFolio).then(
				function(data) {
					$scope.isDeletingFolio = false;

					if (data.success) {
						// Remove the user from the array.
						$scope.folios.splice($scope.folios.indexOf($scope.selectedFolio), 1);
