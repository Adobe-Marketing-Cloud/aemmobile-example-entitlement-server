...
app.service("entitlementService", ["$http", "$rootScope", function($http, $rootScope) {
	return {
		...
		addFolio: function(guid, pubName, folioNumber, productId, pubDate) {
			var folio = {
				guid: guid, pubName: pubName, folioNumber: folioNumber, productId: productId, pubDate: pubDate, csrfToken: $rootScope.csrfToken
			};
			var xsrf = $.param(folio);
			return $http({
					method: "POST",
					url: "php/addFolio.php",
					data: xsrf,
					headers: {"Content-Type": "application/x-www-form-urlencoded"}
				}).then(function(result) {
					return result.data;
		...
		deleteFolio: function(guid, group) {
			console.log(guid, group);
			var xsrf = $.param({guid: guid, productId: group.productId, csrfToken: $rootScope.csrfToken});
			return $http({
					method: "POST",
					url: "php/deleteFolio.php",
					data: xsrf,
					headers: {"Content-Type": "application/x-www-form-urlencoded"}
				}).then(function(result) {
					return result.data;
